...
class FeatureIndex(indexes.SearchIndex, indexes.Indexable):
    ...
    def prepare_orthologs_biomaterial(self, obj):
        ...
        result = list()
        try:
            ortholog_group = Featureprop.objects.get(type__cv__name="feature_property", type__name="orthologous group", feature=obj).value
        except ObjectDoesNotExist:
            return result

        protein_ids = Featureprop.objects.filter(type__cv__name="feature_property", type__name="orthologous group", value=ortholog_group).values_list('feature_id', flat=True)

        for mrna_protein_obj in FeatureRelationship.objects.filter(type__name='translation_of', object_id__in=protein_ids):
    ...
    def prepare_orthologs_coexpression(self, obj):
        ...
        for mrna_protein_obj in FeatureRelationship.objects.filter(type__name='translation_of', object_id__in=protein_ids):
            # mrna_protein_obj.subject == mRNA
            have_coexp = Featureprop.objects.filter(type__cv__name="feature_property", type__name="coexpression group", feature=mrna_protein_obj.subject).exists()
            if have_coexp:
                return True
        return False
