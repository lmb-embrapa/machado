...
class AnalysisTest(TestCase):
    ...
    def test_store_analysis(self):
        ...
        self.assertTrue(Analysis.objects.filter(
            name=test_assayacc1,
            program=test_program,
            programversion=test_programversion).exists())
        self.assertTrue(Analysis.objects.filter(
            name=test_assayacc2,
            program=test_program,
            programversion=test_programversion).exists())
        self.assertTrue(Analysisprop.objects.filter(
            analysis=test_analysis1,
            value=test_filename).exists())
        self.assertTrue(Analysisprop.objects.filter(
            analysis=test_analysis2,
            value=test_filename).exists())
        self.assertTrue(Acquisition.objects.filter(
            assay=test_assay1).exists())
        self.assertTrue(Acquisition.objects.filter(
            assay=test_assay2).exists())
        test_acquisition1 = Acquisition.objects.get(
                assay=test_assay1)
        test_acquisition2 = Acquisition.objects.get(
                assay=test_assay2)
        self.assertTrue(Quantification.objects.filter(
            analysis=test_analysis1,
            acquisition=test_acquisition1).exists())
        self.assertTrue(Quantification.objects.filter(
            analysis=test_analysis2,
            acquisition=test_acquisition2).exists())
        self.assertTrue(Analysisfeature.objects.filter(
            analysis=test_analysis1,
            feature=test_feature1).exists())
        self.assertTrue(Analysisfeature.objects.filter(
            analysis=test_analysis2,
            feature=test_feature2).exists())
        self.assertTrue(Analysisfeature.objects.filter(
            analysis=test_analysis1,
            feature=test_feature1,
            normscore=0.0).exists())
        self.assertTrue(Analysisfeature.objects.filter(
            analysis=test_analysis2,
            feature=test_feature1,
            normscore=0.6936967934559419).exists())
        self.assertTrue(Analysisfeature.objects.filter(
            analysis=test_analysis1,
            feature=test_feature2,
            normscore=2.369615950632963).exists())
        self.assertTrue(Analysisfeature.objects.filter(
            analysis=test_analysis2,
            feature=test_feature2,
            normscore=10.7523002985671).exists())
        self.assertTrue(Analysisprop.objects.filter(
            analysis=test_analysis1,
        ...
        self.assertFalse(Analysisprop.objects.filter(
            analysis=test_analysis1,
            value='exp.matrix.dummy.txt').exists())
        self.assertFalse(Analysisprop.objects.filter(
            analysis=test_analysis2,
        ...
        self.assertFalse(Analysisprop.objects.filter(
            analysis=test_analysis2,
            value=test_filename).exists())
        self.assertFalse(Acquisition.objects.filter(
            assay=test_assay1).exists())
        self.assertFalse(Acquisition.objects.filter(
            assay=test_assay2).exists())
        self.assertFalse(Quantification.objects.filter(
            analysis=test_analysis1,
            acquisition=test_acquisition1).exists())
        self.assertFalse(Quantification.objects.filter(
            analysis=test_analysis2,
            acquisition=test_acquisition2).exists())
        self.assertFalse(Analysisfeature.objects.filter(
            analysis=test_analysis1,
            feature=test_feature1).exists())
        self.assertFalse(Analysisfeature.objects.filter(
            analysis=test_analysis2,
